        ...
        <section className="md:mt-40 md:mb-30 mt-25 mb-20">
            <div className="mx-auto max-w-2xl text-center">   
                <Fade direction="up" cascade>
                    ...
                    <LineSeparator className="mx-auto bg-[#0281FC] mt-4 mb-5" />
            ...
            <div className="grid ml-4 gap-x-10 md:gap-x-16 lg:gap-x-16 gap-y-5 md:grid-cols-2 grid-cols-2 lg:grid-cols-3 mt-10">
                {items.map((element) => {
                    return (
                    <Fade direction="up">
                        <div key={element.title} className={`group relative md:ml-10 ml-4 h-24 items-center rounded-r-full bg-slate-100 px-8 md:pl-16 pl-8 transition duration-300 hover:bg-[#0281FC] flex`}>
                            <div className="absolute -left-10 flex md:h-20 h-15 w-15  md:w-20 items-center justify-center rounded-full border-2 border-slate-500 bg-white transition duration-300 group-hover:border-[#0281FC]">
                                {element.image? <img  src={element.image} alt={element.title} className="h-full w-full rounded-full object-cover p-1" />:''}
                            </div>
                            <Fade cascade direction="right">
                                <h3 className="md:text-sm text-xs font-bold uppercase tracking-widest text-slate-700 transition group-hover:text-sky-50">{element.title}</h3>
                            </Fade>
                        </div>
                    </Fade>
                    );
