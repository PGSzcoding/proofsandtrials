...
export default function ServiceMontacarga({title,subtitle,items}:ServicioTransporteProps){
    ...
    return <>
    <section className=" md:px-20 px-10  pt-17 pb-10 bg-linear-to-r from-slate-950 via-slate-900 to-slate-800 ">
        <div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="min-w-0 md:p-8 p-2">
                <Fade direction="right">
                    <Swiper modules={[Pagination]} pagination={{ clickable: true }} spaceBetween={24} slidesPerView={1} className="w-full">
                        {items.map((item, index) => (
                            <SwiperSlide key={index}>
                                <div className="flex justify-center">
                                    <div className=" max-w-md  group flex min-h-95 flex-col rounded-3xl border border-slate-800 bg-slate-800 p-8 shadow-lg transition-all duration-300">
